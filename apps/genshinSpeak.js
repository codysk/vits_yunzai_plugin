...
import { appCfg } from '../config/config.js'

const require = createRequire(import.meta.url)
const { exec } = require('child_process')


const pythonExecutor = appCfg.vitsPythonPath
...
export class genshinSpeak extends plugin {
  ...
  async genshinSpeak(e) {
    ...
    if (genshinSpeakers.includes(data[0])) {
      ...
      data[1] = text.join("")
        ...
        var cmdStr = pythonExecutor + ' ./plugins/vits_yunzai_plugin/vits/'+ py +' --character=' + characternum + ' --text=' + data[1];
	...
	//bh3音色 author：sumght
      ...
      data[1] = text.join("")
		...
		if(gpu==0){py='run_new.py'};
		if(gpu==1){py='run_old.py'};
        var cmdStr = pythonExecutor + ' ./plugins/vits_yunzai_plugin/vits_bh3/'+ py +' --character=' + characternum_bh3 + ' --text=' + data[1];
        exec(cmdStr, async function (error, stdout, stderr) {
            if (error) {
                console.log("生成失败", stderr);
                e.reply(["生成失败，失败原因:"+stderr])
  ...
  async speakerList(e) {
      e.reply(segment.image('plugins/vits_yunzai_plugin/help/help.jpg'))
    return true
  }
